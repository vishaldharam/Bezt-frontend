    ...
    <div className="absolute modal-container sm:px-0 px-6  inset-0 flex justify-center top-[100px] ">
        <div className="w-[450px]  rounded-lg bg-[#ffff] border-2 border-black/25 h-80    justify-center">
            ...
            <div className="flex w-full space-x-2    justify-center ">
                    ...
                    <div><input readOnly type="text" required value={username} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setUsername(e.target.value)} placeholder="john@gmail.com" className="border-2 border-black/50 px-4 rounded-md py-1.5 w-[280px]" /></div>
                </div>
                ...
                <div className="flex w-[200px]    ml-10 space-x-[30px]">
                    <button onClick={()=> { 
                        setIsEditUserModal(false);
                        setCurrentUser(null)
                        }} className="border border-black/70 bg-white px-4 text-md  rounded-md py-1.5">Cancel</button>
                    <button  onClick={(e: React.MouseEvent<HTMLButtonElement>) => handleSubmit(e)} className="border border-black/70 bg-green-200 px-6 text-md rounded-md py-1.5">Update</button>
                </div>
            </div>
        </div>
    </div>
